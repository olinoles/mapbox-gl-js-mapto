...
export function evaluateVariableOffset(
    anchor: TextAnchor,
...
export function performSymbolLayout(
    ...
    if (bucket.textSizeData.kind === "composite") {
        ...
        sizes.compositeTextSizes = [
            unevaluatedLayoutValues["text-size"].possiblyEvaluate(
                new EvaluationParameters(minZoom),
                canonical
            ),
            unevaluatedLayoutValues["text-size"].possiblyEvaluate(
                new EvaluationParameters(maxZoom),
                canonical
            ),
        ];
    }
    ...
    if (bucket.iconSizeData.kind === "composite") {
        ...
        sizes.compositeIconSizes = [
            unevaluatedLayoutValues["icon-size"].possiblyEvaluate(
                new EvaluationParameters(minZoom),
                canonical
            ),
            unevaluatedLayoutValues["icon-size"].possiblyEvaluate(
    ...
    sizes.layoutTextSize = unevaluatedLayoutValues[
        "text-size"
    ].possiblyEvaluate(new EvaluationParameters(tileZoom + 1), canonical);
    sizes.layoutIconSize = unevaluatedLayoutValues[
        "icon-size"
    ].possiblyEvaluate(new EvaluationParameters(tileZoom + 1), canonical);
    sizes.textMaxSize = unevaluatedLayoutValues["text-size"].possiblyEvaluate(
        new EvaluationParameters(18),
        canonical
    );
    ...
    for (const feature of bucket.features) {
        const fontstack = layout
            .get("text-font")
            .evaluate(feature, {}, canonical)
            .join(",");
        const layoutTextSizeThisZoom = textSize.evaluate(
            feature,
            {},
            canonical
        );
        const layoutTextSize = sizes.layoutTextSize.evaluate(
            feature,
            {},
            canonical
        );
        const layoutIconSize = sizes.layoutIconSize.evaluate(
            feature,
            {},
            canonical
        );
        ...
        if (text) {
            ...
            const spacing =
                layout
                    ...
                    .evaluate(feature, {}, canonical) * ONE_EM;
            const lineHeight =
                layout
                    .get("text-line-height")
                    .evaluate(feature, {}, canonical) * ONE_EM;
            const spacingIfAllowed = allowsLetterSpacing(unformattedText)
                ? spacing
                : 0;
            ...
            const addVerticalShapingIfNeeded = (textJustify: TextJustify) => {
                if (
                    ...
                    allowsVerticalWritingMode(unformattedText)
                ) {
            ...
            if (!textAlongLine && variableTextAnchor) {
                const justifications =
                    textJustify === "auto"
                        ? variableTextAnchor.map((a) =>
                              getAnchorJustification(a)
                          )
                        : [textJustify];
                ...
                for (let i = 0; i < justifications.length; i++) {
                    ...
                    if (singleLine) {
                        ...
                        shapedTextOrientations.horizontal[justification] =
                            shapedTextOrientations.horizontal[0];
                    } else {
                ...
                if (
                    isPointPlacement ||
                    layout.get("text-writing-mode").indexOf("horizontal") >=
                        0 ||
                    !allowsVerticalWritingMode(unformattedText)
                ) {
                    const shaping = shapeText(
                    ...
                    if (shaping)
                        shapedTextOrientations.horizontal[textJustify] =
                            shaping;
                }

                // Vertical point label (if allowVerticalPlacement is enabled).
                addVerticalShapingIfNeeded(
                    isPointPlacement ? "left" : textJustify
                );
            }
        }
 ...
 * @private
 */
function tilePixelRatioForSymbolSpacing(
...
function addFeature(
    ...
    const layout = bucket.layers[0].layout;
    const iconOffset = layout
        .get("icon-offset")
        .evaluate(feature, {}, canonical);
    const defaultShaping =
        getDefaultHorizontalShaping(shapedTextOrientations.horizontal) ||
        shapedTextOrientations.vertical;
    ...
    const glyphSize = ONE_EM,
        ...
        symbolMinDistance =
            tilePixelRatioForSymbolSpacing(bucket.overscaling, bucket.zoom) *
            layout.get("symbol-spacing"),
    ...
    const addSymbolAtAnchor = (
        ...
        if (isGlobe) {
            const { x, y, z } = projection.projectTilePoint(
                anchor.x,
                anchor.y,
                canonicalId
            );
    ...
    if (symbolPlacement === "line") {
        ...
        for (const polygon of classifyRings(feature.geometry, 0)) {
            ...
            addSymbolAtAnchor(
                polygon[0],
                new Anchor(poi.x, poi.y, 0, 0, undefined),
                canonical
            );
        }
        ...
        for (const line of feature.geometry) {
            addSymbolAtAnchor(
                line,
                new Anchor(line[0].x, line[0].y, 0, 0, undefined),
                canonical
            );
        }
        ...
        for (const points of feature.geometry) {
            for (const point of points) {
                addSymbolAtAnchor(
                    [point],
                    new Anchor(point.x, point.y, 0, 0, undefined),
                    canonical
                );
            }
        }
...
function addTextVertices(
    ...
    if (sizeData.kind === "source") {
        textSizeData = [
            SIZE_PACK_FACTOR *
                layer.layout.get("text-size").evaluate(feature, {}, canonical),
        ];
        ...
        if (
            textSizeData[0] > MAX_PACKED_SIZE ||
            textSizeData[1] > MAX_PACKED_SIZE
        ) {
            warnOnce(
                `${bucket.layerIds[0]}: Value for "text-size" is >= ${MAX_GLYPH_ICON_SIZE}. Reduce your "text-size".`
            );
    ...
    for (const placementType of placementTypes) {
        placedTextSymbolIndices[placementType] =
            bucket.text.placedSymbolArray.length - 1;
    }
...
function addSymbol(
    ...
    if (bucket.allowVerticalPlacement && shapedTextOrientations.vertical) {
        ...
        if (textAlongLine) {
            verticalTextCircle =
                evaluateCircleCollisionFeature(verticalShaping);
            if (verticallyShapedIcon) {
                verticalIconCircle =
                    evaluateCircleCollisionFeature(verticallyShapedIcon);
            }
        } else {
            const textRotation = layer.layout
                .get("text-rotate")
                .evaluate(feature, {}, canonical);
            const verticalTextRotation = textRotation + 90.0;
            verticalTextBoxIndex = evaluateBoxCollisionFeature(
                collisionBoxArray,
                collisionFeatureAnchor,
                anchor,
                featureIndex,
                sourceLayerIndex,
                bucketIndex,
            ...
            if (verticallyShapedIcon) {
                verticalIconBoxIndex = evaluateBoxCollisionFeature(
                    collisionBoxArray,
                    collisionFeatureAnchor,
                    anchor,
                    featureIndex,
    ...
    if (shapedIcon) {
        ...
        const verticalIconQuads = verticallyShapedIcon
            ...
            : undefined;
        iconBoxIndex = evaluateBoxCollisionFeature(
            collisionBoxArray,
            collisionFeatureAnchor,
            anchor,
            featureIndex,
        ...
        if (sizeData.kind === "source") {
            ...
            iconSizeData = [
                SIZE_PACK_FACTOR *
                    sizes.compositeIconSizes[0].evaluate(
                        feature,
                        {},
                        canonical
                    ),
                SIZE_PACK_FACTOR *
                    sizes.compositeIconSizes[1].evaluate(
                        feature,
                        {},
                        canonical
                    ),
            ...
            if (
                iconSizeData[0] > MAX_PACKED_SIZE ||
                iconSizeData[1] > MAX_PACKED_SIZE
            ) {
                warnOnce(
                    `${bucket.layerIds[0]}: Value for "icon-size" is >= ${MAX_GLYPH_ICON_SIZE}. Reduce your "icon-size".`
                );
        ...
        if (verticalIconQuads) {
            ...
            verticalPlacedIconSymbolIndex =
                bucket.icon.placedSymbolArray.length - 1;
        }
    }
    ...
    for (const justification: any in shapedTextOrientations.horizontal) {
        ...
        if (!textBoxIndex) {
            ...
            if (textAlongLine) {
                ...
                const textRotate = layer.layout
                    .get("text-rotate")
                    .evaluate(feature, {}, canonical);
                textBoxIndex = evaluateBoxCollisionFeature(
                    collisionBoxArray,
                    collisionFeatureAnchor,
                    anchor,
                    featureIndex,
        ...
        numHorizontalGlyphVertices += addTextVertices(
            ...
            shapedTextOrientations.vertical
                ? WritingMode.horizontal
                : WritingMode.horizontalOnly,
            singleLine
                ? (Object.keys(shapedTextOrientations.horizontal): any)
                : [justification],
    ...
    const getCollisionCircleHeight = (
        diameter: ?number,
        prevHeight: number
    ): number => {
    ...
    collisionCircleDiameter = getCollisionCircleHeight(
        textCircle,
    ...
    bucket.symbolInstances.emplaceBack(
        ...
        placedTextSymbolIndices.center >= 0
            ? placedTextSymbolIndices.center
            : -1,
        placedTextSymbolIndices.left >= 0 ? placedTextSymbolIndices.left : -1,
        placedTextSymbolIndices.vertical >= 0
            ? placedTextSymbolIndices.vertical
            : -1,
        placedIconSymbolIndex,
        verticalPlacedIconSymbolIndex,
        key,
        ...
        textBoxIndex !== undefined
            ? textBoxIndex + 1
            : bucket.collisionBoxArray.length,
        verticalTextBoxIndex !== undefined
            ? verticalTextBoxIndex
            : bucket.collisionBoxArray.length,
        verticalTextBoxIndex !== undefined
            ? verticalTextBoxIndex + 1
            : bucket.collisionBoxArray.length,
        iconBoxIndex !== undefined
            ? iconBoxIndex
            : bucket.collisionBoxArray.length,
        iconBoxIndex !== undefined
            ? iconBoxIndex + 1
            : bucket.collisionBoxArray.length,
        verticalIconBoxIndex
            ? verticalIconBoxIndex
            : bucket.collisionBoxArray.length,
        verticalIconBoxIndex
            ? verticalIconBoxIndex + 1
            : bucket.collisionBoxArray.length,
        featureIndex,
